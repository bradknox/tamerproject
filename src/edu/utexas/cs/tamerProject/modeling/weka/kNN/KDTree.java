...
/*
 * 	  Adapted by W. Bradley Knox from the Weka 3.6.5 version.
 * 
...
import edu.utexas.cs.tamerProject.modeling.weka.kNN.TreePerformanceStats;
...
public class KDTree
  ...
  protected void findNearestNeighbours(Instance target, KDTreeNode node, int k,
    ...
    if (node.isALeaf()) {
      ...
      for (int idx = node.m_Start; idx <= node.m_End; idx++) {
    	  if (m_TreeStats != null) {
    		  m_TreeStats.lastSearchInstancesInOrder.add(m_Instances.instance(m_InstList[idx]));
    	  }
        if (target == m_Instances.instance(m_InstList[idx])) // for
                                                              // hold-one-out
                                                              // cross-validation
          continue;
        if (heap.size() < k) { // trivial case
  ...
  public Instances kNearestNeighbours(Instance target, int k) throws Exception {
    ...
    if (m_Stats != null)
        m_Stats.identifyInstanceLocsInSearch(neighbours);
    return neighbours;
  }
