 ...
 */
public class PerformanceStats extends weka.core.neighboursearch.PerformanceStats
  ...
  public int[] lastSearchOrderOfKNeighbors;
  public ArrayList<Instance> lastSearchInstancesInOrder;
  
  public void identifyInstanceLocsInSearch(Instances instances){
	  if (lastSearchInstancesInOrder != null && lastSearchInstancesInOrder.size() > 0) {
		  lastSearchOrderOfKNeighbors = new int[instances.numInstances()];
		  for (int i = 0; i < instances.numInstances(); i++) {
			  lastSearchOrderOfKNeighbors[i] = findIndexOfInstance(lastSearchInstancesInOrder, instances.instance(i));
		  }
	  }
  }
  
  private int findIndexOfInstance(ArrayList<Instance> instanceList, Instance inst) {
	  for (int i= 0; i < instanceList.size(); i++) {
		  Instance comparisonInstance = instanceList.get(i);
		  boolean allInstancesSame = true;
		  for (int j = 0; j < inst.numAttributes(); j++) {
			  if (inst.value(j) != comparisonInstance.value(j)) {
				  allInstancesSame = false;
				  break;
			  }
		  }
		  if (allInstancesSame)
  ...
  public void searchStart() {
    m_PointCount = 0;
    m_CoordCount = 0;
    lastSearchInstancesInOrder = new ArrayList<Instance>();
  }
