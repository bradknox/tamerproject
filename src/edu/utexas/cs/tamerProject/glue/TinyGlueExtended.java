/*
I (Brad Knox) changed this code from the original TinyGlue code. The only
reason it's more or less copied here is that Linux mysteriously pulls
the old version of TinyGlue when running the applet, causing an 
exception. OS X and Windows see the new version, and from what I can
tell, there is only one TinyGlue.class in the applet's jar file. So
having this class here is a hack to get around my Linux incompatibility.
This hack should be removed at some point.
...
public class TinyGlueExtended extends TinyGlue {
	...
	private boolean stepEnvAgent(){
        ...
        if (RLGlue.isCurrentEpisodeOver()) {
            ...
            if (ROT.isTerminal()) {
            	double r = ROT.getReward();
            	System.out.println("Glue sending reward to agent_end(): " + r);
                RLGlue.RL_agent_end(r);
            } else {
     ...
     * 
     * Returns whether agent_end() was called.
     */
    private  boolean  stepAgentEnv() {
    	boolean agentEndCalled = false;
		 ...
		 */
			...
			synchronized (this) {
				...
				this.lastEnvStepTerminal = ROT.isTerminal();
