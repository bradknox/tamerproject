...
import edu.utexas.cs.tamerProject.environments.robotarm.messages.StateResponse;

import org.rlcommunity.rlglue.codec.types.Action;
import org.rlcommunity.rlglue.codec.types.Observation;
import org.rlcommunity.rlglue.codec.types.Reward_observation_terminal;

import edu.utexas.cs.tamerProject.environments.robotarm.visualizer.RobotArmVisualizer;
...
public class RobotArm extends SampleableEnvBase implements
    ...
    protected final RobotArmState theState;
    ...
    public TaskSpec makeTaskSpec() {
        ...
        for (int i = 0; i < RobotArmState.worldDims.length; i++) {
        	theTaskSpecObject.addDiscreteObservation(new IntRange(0, RobotArmState.worldDims[i] - 1));
        }
        //theTaskSpecObject.addContinuousObservation(new DoubleRange(0, theState.worldDims[0]));
        
        int numActions = RobotArmState.worldDims.length * 2;
        theTaskSpecObject.addDiscreteAction(new IntRange(0, numActions - 1));
        theTaskSpecObject.setRewardRange(new DoubleRange(-80, 0));
        theTaskSpecObject.setExtra("EnvName:RobotArm Revision:" + this.getClass().getPackage().getImplementationVersion());
    ...
    public Reward_observation_terminal env_step(Action theAction) {
        ...
        int maxPossibleAct = (RobotArmState.worldDims.length * 2) - 1;
        ...
        //System.out.println("Action in RobotArm.env_step(): " + a);
        theState.update(a);
...
//        }
     ...
     * @param p
     */
    public RobotArm(ParameterHolder p) {
    	this(p, false);
    }
    
    public RobotArm(ParameterHolder p, boolean allowSecretPaths) {
        super();
        ...
        theState = new RobotArmState(randomStartStates, transitionNoise, randomSeed, allowSecretPaths);
	...
	private void processArgs(String[] args) {
		System.out.println("\n[------RobotArm command line args------] " + Arrays.toString(args));
...
//			}
    ...
    public String env_message(String theMessage) {
        ...
        if (theMessageObject.getTheMessageType() == rlVizLib.messaging.environment.EnvMessageType.kEnvCustom.id()) {
            ...
            if (theCustomType.equals("GETPWSTATE")) {
                ...
                int[] agentPosition=RobotArmState.getDefaultInitPosition();
                if(theState!=null){
        ...
        System.err.println("We need some code written in Env Message for RobotArm.. unknown request received: " + theMessage);
        Thread.dumpStack();
        return null;
    }
    ...
    public double getMaxValueForQuerableVariable(int dimension) {
    	return RobotArmState.worldDims[dimension];
    }
    ...
    public int getNumVars() {
        return RobotArmState.worldDims.length;
    }

    public String getVisualizerClassName() {
        return RobotArmVisualizer.class.getName();
    }
	...
	public Reward_observation_terminal sampleStartObs(boolean envAsking) {
		...
		rot.o = RobotArmState.sampleEnvStart(envAsking);
		rot.r = 0;
		rot.terminal = 0;
		return rot;
	}
	...
	public Reward_observation_terminal sampleNextObs(Observation obs,
			Action act) {
		Reward_observation_terminal rot=new Reward_observation_terminal();
		rot.o = RobotArmState.sampleNextObs(obs, act);
		rot.terminal = 0;

		/*
		 * Get reward
		 */
		rot.r = RobotArmState.getReward(rot.o.intArray);
		
		
		/*
         * Check for terminal state
         */
        if (RobotArmState.inGoalRegion(rot.o.intArray))
        	rot.terminal = 1;
	...
	public boolean isObsLegal(Observation obs) {
		return RobotArmState.isStateLegal(obs.intArray);
	}

	@Override
	public boolean isObsTerminal(Observation obs) {
		return RobotArmState.inGoalRegion(obs.intArray);
	}
}
